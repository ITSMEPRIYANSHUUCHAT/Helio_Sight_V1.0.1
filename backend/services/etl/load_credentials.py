...
def get_valid_customer_ids(conn):
    try:
        ...
        logger.error(f"Failed to fetch valid customer IDs: {e}")
        return set()
    ...
    try:
        with conn.cursor() as cur:
            cur.execute(
                ...
                """,
                (
                    customer_id, device_sn, datetime.now(timezone('Asia/Kolkata')),
...
def load_credentials_to_db(conn, csv_path):
    try:
        ...
        valid_customer_ids = get_valid_customer_ids(conn)
        if not valid_customer_ids:
            logger.info("No customers to process, exiting.")
            return []
